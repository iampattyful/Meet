...
export class MemoService {
  ...
  private async selectAllMemo(txn: Knex.Transaction): Promise<memo[]> {
    return await txn("memo")
      .select("id", "content", "image", "users_id", "like_count")
  ...
  async likeMemo(id: number, userId: number): Promise<void> {
    ...
    try {
      ...
      // [id,userId,userId])
      let [q]: { like_count: number }[] = await txn
        .select("like_count")
        .from("memo")
        .where("id", id);
      console.log(q, "qqq");
      ...
      //  [id, userId, userId])
      let s = await txn("memo")
        .update({ like_count: q.like_count + 1 })
        .whereNotIn("id", function () {
          this.select("memo_id").from("like_memo").where("users_id", userId);
        })
        .andWhere("id", id)
        .andWhereNot("users_id", userId)
        .returning("id");
      console.log(s, "sssss");
      if (s.length > 0) {
